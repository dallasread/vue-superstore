...
describe('Stores:Local', () => {
  ...
  it('query', async () => {
    ...
    expect(subject.query(model)).resolves.toEqual([])
  })
  ...
  it('find', async () => {
    ...
    expect(subject.find(model, task.id)).resolves.toEqual(task)
  })
  ...
  it('save', async () => {
    ...
    expect(subject.db.getItem(model._key)).resolves.toEqual([task.toJSON()])
  })
