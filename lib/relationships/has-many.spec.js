...
describe('Relationships:HasMany', () => {
  it('default configuration', async () => {
    ...
    const project = superstore.projects.build({ id: id })
    const task = superstore.tasks.build({ projectId: project.id })
  ...
  it('primary key', async () => {
    ...
    const id = Math.random()
    const project = await superstore.projects.save({ customId: id })
    const task = await superstore.tasks.save({ projectId: id })
  ...
  it('foreign key', async () => {
    ...
    const id = Math.random()
    const project = await superstore.projects.save({ id: id })
    const task = await superstore.tasks.save({ customId: id })
