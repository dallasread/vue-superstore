...
const buildModel = (name, options) => {
  ...
  models[name] = new MultiStorage(reactive, models, name, options)
...
describe('Models:MultiStorage', () => {
  it('requires stores', () => {
    expect(() => buildModel('project')).toThrow(new Error('`MultiStorage.options.stores` is required'))
  })

  it('builds out configured stores', () => {
    const subject = buildModel('project', { stores: { local: { type: 'Local', name: 'local' }, remote: { type: 'Rest', url: 'local' } } })
  ...
  it('builds default strategies', () => {
    const subject = buildModel('project', { stores: { local: new FakeLocal(), remote: new FakeRemote() } })
