...
const Base = function (options) {
  ...
  options.methods.changes = function () {
  ...
  options.methods.init = function () {
    this.updateLastSaved()
  }

  options.methods.rollback = function () {
  ...
  options.methods.hasChanges = function () {
    return !!Object.keys(this.changes()).length
  }

  options.methods.toJSON = function () {
    const obj = { id: this.id }

    if (options.props instanceof Array) {
      for (let i = 0; i < options.props.length; i++) {
        obj[options.props[i]] = this[options.props[i]]
      }
    } else if (typeof options.props === 'object') {
      for (const key in options.props) {
        if (typeof this[key] === 'undefined') {
          obj[key] = options.props[key].default
        } else {
          obj[key] = this[key]
        }
      }
  ...
  options.methods.updateLastSaved = function (data) {
    return this._lastSave = data || JSON.stringify(this.toJSON())
  }

  options.methods.save = function () {
  ...
  options.methods.destroy = function () {
    return new Promise((resolve) => {
