...
describe('Models:Base', () => {
  it('initializes', () => {
    ...
    const subject = new Base({ name, computed, mixins, methods, relationships })
  ...
  describe('crud', () => {
    ...
    it('build', () => {
      ...
      const obj = subject.data.build(task)
    ...
    it('find', async () => {
      const task = subject.data.build({ id: 5, name: 'Task' })

      subject.data.push(task)

      await expect(subject.find(task.id)).resolves.toEqual(task)
    })
    ...
    it('save should update its last save', () => {
      const task = subject.data.build({ id: 5 })
