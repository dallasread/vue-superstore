...
const buildModel = (name, options) => {
  ...
  models[name] = new Base(reactive, models, name, options)
...
describe('Models:Base', () => {
  ...
  it('computes toJSON based on props', () => {
    const subject = buildModel('project', {
      props: ['name']
    })
  ...
  it('sets the default foreign key', async () => {
    const key = 'asdf'
    const subject = buildModel('project', { defaultForeignKey: key })
  ...
  describe('save', () => {
    ...
    it('modifies the instance in the array', async () => {
      const subject = buildModel('project', { props: ['name'] })
  ...
  describe('find', () => {
    ...
    it('uses a custom primary key', async () => {
      const key = 433
      const subject = buildModel('project', { primaryKey: 'key' })
