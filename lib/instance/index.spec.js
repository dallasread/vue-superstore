...
const buildModels = (name, opts) => {
  ...
  models[name] = new Model(reactive, computed, models, name, opts)
...
describe('Instance', () => {
  ...
  it('applies custom computed methods with correct this', () => {
    ...
    expect(subject.test.value).toEqual(subject)
  })

  it.todo('applies custom computed methods that change'/*, () => {
    const modelName = 'projects'
    const models = buildModels(modelName, { props: ['name'], computed: { test () { return this.name } } })
    const subject = new Instance(models, modelName)

    subject.name = 'Bob'
    expect(subject.test.value).toEqual(subject.name)
