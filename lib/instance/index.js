...
const createFunc = (instance, func) => {
  return function _createFunc () {
    return func.call(instance, ...arguments)
  }
}
...
class Instance {
  constructor (models, modelName, attrs) {
    ...
    this.applyComputed(this.model.computed)
  ...
  applyComputed (computed) {
    for (const key in computed) {
      Object.defineProperty(this, key, {
        get: createFunc(this, computed[key]),
        set (arr) {
          throw new Error('Cannot write to computed property.')
        }
      })
    }
  }
